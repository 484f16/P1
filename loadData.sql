...
insert into users 
	(select user_id,first_name,last_name,year_of_birth,month_of_birth,day_of_birth,gender from t2);	
...
create table temp_events as 
	(select T.event_id,T.event_creator_id,T.event_name,T.event_tagline,T.event_description,T.event_host,T.event_type,T.event_subtype,T.event_location,
		C.city_id, T.event_start_time, T.event_end_time from cities C,te T
